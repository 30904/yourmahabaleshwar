...
export default function HomestayRegistrationFields({ form, setField, toggleAmenity, isEdit = false }) {
  ...
  const termsSummary = t('homestayRegistration.termsSummary', { returnObjects: true });
  const termList = Array.isArray(termsSummary) ? termsSummary : [];
  const fullTermsSections = t('homestayRegistration.fullTermsSections', { returnObjects: true });
  ...
  return (
    <>
      <Card className="space-y-4">
        <SectionTitle>{t('homestayRegistration.section1')}</SectionTitle>
        <div className="grid gap-4 sm:grid-cols-2">
          <Input
            className="sm:col-span-2"
            label={t('homestayRegistration.officialName')}
            value={form.name}
          ...
          />
          <PincodeInput label={t('stayRegistration.pinCode')} value={form.pincode} onChange={(e) => setField('pincode', e.target.value)} />
          <Input
            className="sm:col-span-2"
      ...
      <Card className="space-y-4">
        <SectionTitle>{t('homestayRegistration.section5')}</SectionTitle>
        ...
        <div className="flex flex-wrap gap-3 text-sm">
          <button type="button" className="font-semibold text-primary underline" onClick={() => setLegalOpen(true)}>
        ...
        <label className="flex items-start gap-2 text-sm text-slate-700">
          ...
          />
          <span>{t('homestayRegistration.acceptTerms')}</span>
        </label>
      </Card>
      ...
      <LegalModal
        open={legalOpen}
        title={t('homestayRegistration.fullTermsTitle')}
