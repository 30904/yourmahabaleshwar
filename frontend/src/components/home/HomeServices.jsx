...
export default function HomeServices() {
  ...
  return (
    <section className="page-container py-12 sm:py-16">
      <HomeSectionHeader
        eyebrow="Beyond stays"
        title="Guides & transport"
        subtitle="Explore Mahabaleshwar with trusted locals"
        linkTo="/guides"
      />
      <div className="mt-8 grid gap-4 md:grid-cols-2">
        <Link to={`/guides/${guide.slug}`} className="home-service-card">
          ...
          <div className="flex flex-1 flex-col p-5 sm:p-6">
            <span className="home-service-tag">Local guide</span>
            ...
            <p className="mt-auto pt-4 text-lg font-bold text-primary">
              From {formatCurrency(guide.package6hr)}
              <span className="text-sm font-normal text-slate-500"> / 6 hrs</span>
            </p>
          </div>
        </Link>
        <Link to={`/taxi/${driver.slug}`} className="home-service-card">
          ...
          <div className="flex flex-1 flex-col p-5 sm:p-6">
            <span className="home-service-tag">Taxi & cab</span>
            ...
            <p className="mt-3 flex items-center gap-1 text-sm text-slate-600">
              <Clock size={14} />
              Hourly & point-to-point trips
            </p>
            <p className="mt-auto pt-4 text-lg font-bold text-primary">
              From {formatCurrency(driver.perTripPrice)}
              <span className="text-sm font-normal text-slate-500"> / trip</span>
            </p>
          </div>
        </Link>
      </div>
      <div className="mt-4 flex flex-wrap justify-center gap-3">
        <Link to="/guides" className="btn-outline"><Users size={18} />Browse all guides</Link>
        <Link to="/taxi" className="btn-outline"><Car size={18} />Book a cab</Link>
      </div>
    </section>
  );
}
