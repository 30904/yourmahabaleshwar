...
function LegalModal({ open, onClose }) {
  if (!open) return null;
  const doc = HOMESTAY_TERMS_AND_CONDITIONS;
  return (
...
export default function HomestayGuestBookingForm({ item }) {
  ...
  const room = useMemo(
    () => (item?.rooms || []).find((r) => String(r._id) === String(form.roomId)) || item?.rooms?.[0],
    [item, form.roomId]
  );
  ...
  const validate = () => {
    ...
    if (!form.roomId && !item?.rooms?.[0]?._id) return 'Please select a room';
    ...
    <>
      <form onSubmit={onSubmit} className="space-y-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Homestay booking Form</h2>
            <p className="mt-1 text-sm text-slate-500">S.M. Enterprises — guest details for {item?.name}</p>
          </div>
